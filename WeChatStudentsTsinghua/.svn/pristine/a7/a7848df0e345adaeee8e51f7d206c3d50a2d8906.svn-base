...
import students.tsinghua.service.MovieService;
...
/**
 * filter whether the message is for movie
 * 
 * @author hujiawei
 * 
 */
public class FilterMovie extends MessageFilterHelper implements IMessageFilter {
	...
	public Message doSpecailMessageFilter(Message message) {
		if (message instanceof MessageText) {// it just handle text message
			...
			if (Pattern.compile(ResourceManager.getValue("movie")).matcher(messageText.getContent()).find()) {
				List<ItemArticle> articles;
				try {
					articles = new MovieService().getItemList();
